...
class UserProfileAllergicAdapter :
    ...
    class UserProfileAchievmentViewHolder(private val binding: ItemUserAllergyBinding) :
        ...
        fun bind(allergy: Allergy) {
            binding.allergicText.text = allergy.name
            binding.allergicImage.setImageResource(allergy.image)
            val context = itemView.context
            if (allergy.isSelected) {
                itemView.setBackgroundColor(ContextCompat.getColor(context, R.color.white))
            } else {
                itemView.setBackgroundColor(ContextCompat.getColor(context, R.color.color_background))
            }
